...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(hidden_size,len(vocab.tgt.word2id),bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        cat_last_hidden = torch.cat((last_hidden[0],last_hidden[1]),1)
        cat_last_cell = torch.cat((last_cell[0],last_cell[1]),1)
        init_decoder_hidden = self.h_projection(cat_last_hidden)
        init_decoder_cell = self.c_projection(cat_last_cell)
